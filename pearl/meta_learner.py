...
class MetaLearner:
    ...
    def collect_train_data(
        ...
        while cur_samples < max_samples:
            trajs, num_samples = self.sampler.obtain_samples(
                ...
                accum_context=False,
                # 初始化时的accum_context是False, 即不对context进行拼接,
                # 此时的agent.encoder.context = None
            )
            # sample就是transitions，sample的数量就是trans的数量
            cur_samples += num_samples

            # Add collected trajectory to the RL replay buffer
            self.rl_replay_buffer.add_trajs(task_index, trajs)
            # 上一步返回的trajs是一个dict的list，此时要将其解包
    ...
    def meta_train(self) -> None:
        ...
        for iteration in range(self.num_iterations):
            # self.num_interation = args_pearl.num_epochs
            start_time: float = time.time()

            if iteration == 0:
                # 在第一次迭代时，对所有任务收集一定量的trajs,
                # 以免第一次随机采样未覆盖, 导致第一次训练数据缺失
                ...
                for index in tqdm(self.train_tasks):
                    ...
                    self.collect_train_data(
                        ...
                        update_posterior=False,  # 不更新后验
                        add_to_enc_buffer=True,
                    )
                    # 初次迭代时，用初始policy对任务分布上的每个任务收集一定量的samples，
                    # 将trajs加入到encoder的全局buffer中和rl的全局buffer中。
                    # 初始化不收集context，也不更新后验q(z|c)
                    # 交互过程中所使用的的z都是N(0,1)上采样得来的
            ...
            for i in range(self.num_sample_tasks):
                ...
                # 清除specific-task的encoder buffer，初始化时的rl buffer没动
